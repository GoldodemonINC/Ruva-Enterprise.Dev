...
pub struct CodeGen {
    output: String,
    indent: usize,
    /// Mapping from ruva:: imports to actual Rust crate imports
    import_map: std::collections::HashMap<String, String>,
    /// Collected external crate dependencies (name, version)
...
impl CodeGen {
    ...
    /// Build the mapping from ruva:: imports to actual Rust crate imports
    fn build_import_map() -> std::collections::HashMap<String, String> {
        let mut map = std::collections::HashMap::new();

        // Graphics - OpenGL (using glutin + gl crates)
        ...
        // Anticheat (using winapi on Windows, nix on Unix)
...
"#,
    ...
    pub fn generate_rust(&mut self, program: &Program) -> String {
        ...
        // Process imports first to collect dependencies
        ...
        // Generate external crate imports if we have dependencies
    ...
    fn gen_import(&mut self, imp: &ImportDef) {
        ...
        if imp.path.starts_with("ruva::") {
            if let Some(crate_name) = self.import_map.get(&imp.path) {
                ...
                // For module imports, we don't generate a use statement here
                // The actual use statements are generated at the top
            }
        } else {
            // Regular Rust imports
            if let Some(ref alias) = imp.alias {
    ...
    fn gen_stmt(&mut self, stmt: &Stmt) {
        match stmt {
            ...
            Stmt::For { pattern, iterable, body } => {
                ...
                // For array literals, add .iter() so Rust can iterate
            ...
            Stmt::TryCatch { try_body, catch_param, catch_body } => {
                // Transpile try/catch to match on Result
                self.writeln("{");
                self.indent += 1;
                self.writeln(&format!("let _ruva_result = (|| -> Result<(), String> {{"));
                self.indent += 1;

                // Generate try body with ? on fallible expressions
    ...
    fn gen_expr(&mut self, expr: &Expr) {
        match expr {
            ...
            Expr::Float(f) => {
                // Always emit a decimal point so Rust sees a float, not an int
                let s = format!("{}", f);
            ...
            Expr::StructLiteral { name, fields } => {
                // Handle Self as special case (needs uppercase)
    ...
    // Helper for formatting with write!
    fn write(&mut self, s: &str) {
        self.output.push_str(s);
    }

    /// Generate an expression to a temporary string without allocating a new CodeGen.
